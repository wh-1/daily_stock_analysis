...
> **边界声明**：作者另有一独立项目 **「三态攻守系统」**（A股单一策略——三态 BEAR/MIX/STRONG + 低吸 v6 的量化研究 + 每日盘后自动化）。该项目与本项目的定位不同：本项目是**多市场通用 AI 分析产品**（含 Web/桌面端/推送），策略层为 15 套通用技术形态模板；「三态/低吸 v6」是三态攻守系统的独有锁定引擎，本项目**不内置、不共享**其策略内核。两项目独立演进，请勿混淆。
